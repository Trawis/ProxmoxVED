...
> **Unprivileged containers** shift UIDs by 100000. The uid depends on which service
> user you chose during install.

**Dedicated `torrent` user (default, uid 999):**
...
# Privileged container — already owned by root, no chown needed
```
...
| Downloads owned by wrong user | Mount point uid mismatch | `torrent` user: `chown -R 100999:100999` (unprivileged) or `999:999` (privileged); root user: `chown -R 100000:100000` (unprivileged) |
